...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    const user = new User({ name, email, phone_number, password: hashedPassword });
    await user.save();
...
app.post("/expenses", async (req, res) => {
  const { category, description, amount, created_at } = req.body;
  const userId = req.session.user._id;
  try {
    const user = await User.findById(userId);
    if (!user) {
    ...
    user.expenses.push({ category, description, amount, created_at });
    await user.save();
